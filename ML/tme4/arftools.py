...
def plot_frontiere(data,f,step=20,kernel=False):
    ...
    """
    data2D=data[:,:2]
    grid,x,y = make_grid(data=data2D,step=step)
    #!! ICI je reconstruiis la grille pour avoir des données prenant en compte le biais
    # SI je veux faire un kernel trick alors il faut rajouter les autres colonnes
    if not kernel:
    ...
    if kernel:
        grid = np.hstack((
            grid,
            (grid[:,0] * grid[:,0]).reshape(-1,1),
            (grid[:,1] * grid[:,1]).reshape(-1,1),
            (grid[:,0] * grid[:,1]).reshape(-1,1),
            np.ones((len(grid),1))
        ))
